...
import SubmitQuizModal from '../Student/Modal/SubmitQuizModal';
...
function Submission() {
  ...
  const [loading, setLoading] = useState(false); // Start with true loading state
  ...
  const paginatedSubmittedQuizzes = paginateQuizzes(
    submittedQuizzes,
  ...
  const fetchQuizzes = () => {
    ...
    if (batchId) {
      ...
      axios
        ...
        .then((response) => {
          const { newQuizzes, lateQuizzes, submittedQuizzes } =
            response.data.data;
  ...
  const handleViewResults = (quizId) => {
    navigate(
      `/student/quiz/results?quizId=${quizId}&submitterId=${userInfo?._id}`,
    );
  };
  ...
  return (
    ...
    className="mt-4 px-4 py-2 bg-black text-white rounded hover:bg-gray-600 dark:bg-blue-700 dark:hover:bg-blue-800"
  >
    New Button
  </button>
          </div>
