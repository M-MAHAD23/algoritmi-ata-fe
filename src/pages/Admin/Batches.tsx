...
function Batches() {
  ...
  const renderTable = ({ headers, rows, renderRow, keyExtractor }) => (
    <div className="mb-8">
      {/* Table Header */}
      <div
        className={`flex justify-between items-center bg-gray-200 dark:bg-meta-4 rounded-sm`}
      >
      ...
      {rows.length ? (
        rows.map((row) => (
          <div
            ...
            className={`flex justify-between items-center border-b border-stroke dark:border-strokedark`}
          >
