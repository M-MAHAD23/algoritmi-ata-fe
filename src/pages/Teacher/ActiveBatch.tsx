                ...
                {batch ? (
                    <>
                        ...
                        <div className="mt-6 sm:mt-8 md:mt-10 lg:mt-12 xl:mt-16">
                            ...
                            <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                                {renderTable({
                                    headers: ["#ID", "Name", "Ethics Status"],
                                    rows: batch.batchStudent || [],
                                    keyExtractor: (student) => student._id,
                                    renderRow: (student) => (
                                        <>
                                            <div className="p-2.5 text-center">{student.studentId}</div>
                                            <div className="p-2.5 text-center">{student.firstName} {student.lastName}</div>
                                            <div className="p-2.5 text-center">{student.ethics || '-'}</div>
                                        </>
                                    ),
                                })}
                            </div>
                            ...
                            <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 mt-6">
                                {renderTable({
                                    headers: ["#Quiz ID", "Quiz Name", "Status"],
                                    rows: batch.batchQuiz || [],
                                    keyExtractor: (quiz) => quiz._id,
                                    renderRow: (quiz) => (
                                        <>
                                            <div className="p-2.5 text-center">{quiz.quizId}</div>
                                            <div className="p-2.5 text-center">{quiz.quizName}</div>
                                            <div className="p-2.5 text-center">{quiz.status || 'Pending'}</div>
                                        </>
                                    ),
                                })}
                            </div>
                    ...
                    <div className="text-center">Batch details not found.</div>
                )}
            </div>
        </Panel>
    );
